...
buildall()
{
	...
	if [[ "${1}" = "yes" ]]; then
		...
		then
		    ...
		    cp $DST_DIR/cpp/* ../src/protobuf/
		    echo "拷贝到源码目录"

		#python
		    if [ ! -d ../src/python_logic/YouMai ]; then
			echo "创建目录"
			mkdir ../src/python_logic/YouMai
		    else
			echo "清空目录"
			rm -rf  ../src/python_logic/YouMai/*
		    fi		

		    cp $DST_DIR/python/YouMai/*   ../src/python_logic/YouMai
		    echo "拷贝到源码目录"
